...
public class Merge {
     ...
     * @return sub-arrays of ary
     */
    public static Comparable[][] divide(Comparable[] ary) {
        int len = ary.length;
        int mid = len / 2;
        Comparable[] first = new Comparable[mid];
        Comparable[] last = new Comparable[mid + (len %2)];

        int i = 0, j = 0, k = 0;
        while (i < first.length) first[i++] = ary[k++];
        while (j < last.length) last[j++] = ary[k++];

        return new Comparable[][] {first, last};
    }
     ...
     * @param b second half of ary
     */
    public static void merge(Comparable[] ary, Comparable[] a, Comparable[] b) {
        // Loop until we've gone thru sub-arrays a and/or b.
        int i = 0, j = 0, k = 0;
        while (i < a.length && j < b.length)
            ary[k++] = (a[i].compareTo(b[j]) < 0) ? a[i++] : b[j++];
